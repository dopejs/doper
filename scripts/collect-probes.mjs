...
async function handleRequest(request, response, context) {
  ...
  if (
    ...
    response.setHeader("WWW-Authenticate", 'Basic realm="pingo probe collector"');
...
function isAuthorized(request, token) {
  ...
  return secureEqual(decoded, `pingo:${token}`);
}
...
async function main() {
  ...
  options.token = process.env.PINGO_PROBE_COLLECTOR_TOKEN;
  if (!isLoopbackHost(options.host)) {
    ...
    if (options.token === undefined || options.token.length < 24) {
      throw new Error(
        "non-loopback collection requires PINGO_PROBE_COLLECTOR_TOKEN with at least 24 characters",
      );
    }
  }
  ...
  process.stdout.write(
    `pingo probe collector: ${protocol}://${options.host}:${String(options.port)}/?collector=1&deviceId=<asset-id>&roleId=<role-id>\narchive: ${path.resolve(options.output)}\n`,
  );
}
