...
/** Small synchronous SHA-256 used for immutable stylesheet content identity in browsers/workers. */
export function sha256(value: string): string {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    for (let index = 0; index < 16; index += 1) {
      schedule[index] = data.getUint32(offset + index * 4, false);
    }
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 =
        (h + sum1 + choice + (roundConstants[index] ?? 0) + (schedule[index] ?? 0)) >>> 0;
