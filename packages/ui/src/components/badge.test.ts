...
describe("Badge", () => {
  it("composes default variant classes", () => {
    const node = Badge.component({ children: "Beta" }) as { props: Record<string, unknown> };
    expect(node.props.className).toBe("pui-badge pui-badge--default");
  });

  it("supports variants and dark theme", () => {
    setTheme("dark");
    const node = Badge.component({ children: "x", variant: "outline" }) as {
  ...
  it("appends user className last", () => {
    const node = Badge.component({ children: "x", className: "mine" }) as {
      props: Record<string, unknown>;
    };
