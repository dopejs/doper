...
本计划用于把技术设计转成可执行的研发项目。最终交付不是 Faster 的
复刻，而是一套从第一性原理重新设计的 Web Canvas 渲染引擎：

- 保留 Faster 已被业务验证的优势，例如 TSX 开发体验、虚拟滚动、缓存和
  PC 性能。
...
- 提供 canvas 内原生 caret、selection、IME 和文本编辑，业务不再为输入能力
  创建 EmbedDOM 控件。
- 用外围兼容层支持存量迁移，不让 Faster 的内部模型限制 doper Core。

Faster 在本项目中只承担三种角色：黑盒行为基线、性能对照组、迁移输入。
不得以复制其源码、内部抽象或历史兼容行为作为实现捷径。
...
- Faster shim 位于边界层，可独立删除，不污染 Core。
...
- Faster 对照场景、数据集和可稳定重复的 benchmark 环境。
...
> 当前状态：进行中。Rust/TypeScript workspace、基础 CI、最小 WASM 构建和
> M0 浏览器探针已建立；benchmark fixture、设备矩阵和 Faster 黑盒基线尚未完成。
...
- 对 Faster 只做公开行为与黑盒性能采集，形成可追溯基线。
...
> 当前状态：进行中。桌面 Chrome 已完成探针自检，但真机矩阵、业务
> COOP/COEP 结论、三档 transport 原型和多输入法录制回放尚未完成。
...
- 位于边界包的 Faster compatibility shim、迁移指南和自动检查工具。
...
- 至少一个包含真实编辑场景的代表性存量页面完成灰度，并达到正确性、性能
  和稳定性门禁。
- 可按页面退回 Faster，可按能力退回主线程 Canvas2D。
...
| Faster 兼容污染 Core       | 全程     | Core 出现 Faster 专用类型/分支                 | shim 边界隔离、依赖规则、删除性测试                          |
