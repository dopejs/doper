...
> 日期：2026-08-21
>
> 结论：六类候选均为 **Defer**。M9 不实现这些能力；没有真实业务 fixture 和可自动执行的
> 出口 oracle 时，不用“可能有用”扩大 Core、ABI 或产品支持面。
...
| overlay/absolute positioning 与 widgets placeholder      | Defer | 没有覆盖层叠、锚点、焦点和滚动容器的产品 fixture | layout/hit/clip/semantics 增量↔全量 oracle，帧时和节点预算                          | 新 positioning computed value 与失效语义；需要无障碍顺序资格     | feature bit 关闭后拒绝新值，现有 flow layout 保持         |
