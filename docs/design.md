...
- **高精度（触摸板）**：即时 1:1，惯性仍由操作系统的事件流提供，引擎不叠加自己的
  fling，避免双重惯性。
- **离散格**：累加到一个动画目标，由 `ScrollPhysics` 在 `advance()` 里按指数缓出
  逼近（时间常数 45ms，约 135ms 走完 95%）。连续格合并进同一段动画；目标硬夹到
  内容边界，不产生 overscroll，与浏览器一致。直接拖拽、高精度 delta 和程序化
  `jump_to` 都会立即取消未完成的动画。
...
**验证**：`doper-scroll` 的动画累加、边界夹取、取消路径与"小于停止距离立即应用"
各有单测；`doper-core` 有一条引擎级测试断言同一份 delta 在两种标志下分别是即时
