...
**本次只修了 1**：可编辑节点发出自身盒子的 `ClipRect`。装饰（caret、选区、组词下划线）与
文本在同一条指令流里，因此一并被裁剪，行为一致。

**这暴露出两个后续缺口，尚未实现，且第一条是本次修改带来的**：

- **单行框缺少框内水平滚动**。`caret_reveal_target` 只会滚动**最近的 Scroll 祖先**，而可编辑
  节点本身不是 Scroll 节点，所以没有"框内跟随 caret 横向滚动"。裁剪之前，长值溢出到框外
  但看得见；裁剪之后会被切掉且 caret 不可见。原生 input 的做法是会话持有一个局部滚动偏移，
  在 `ClipRect` 之后作为 `Transform` 施加，并同步进命中测试的局部坐标与编辑几何。
- **多行框缺少软换行**。需要在回退路径上按已测量的逐码点推进做贪心断行，并让
  `DrawTextInlineFallback` 承载断行结果（目前后端只按 `\n` 分行绘制），caret 停靠点也要按
  视觉行重建。

**验证**：`doper-paint` 断言可编辑节点会发出 `ClipRect([0,0,w,h])`；把 `NodeKind::EditableText`
从条件里去掉复跑，断言失败。
