...
pnpm m1:check        # 构建、lint、类型、测试、覆盖率门槛、m1 帧时基准
...
`pnpm m9:check` 为最完整的一条链，包含 m0–m8 的全部门禁。下表数字取自同一次执行，
非历史记录：
...
| m1 Core 帧时 | p95 3.180ms（预算 16.7ms）                   |
| m3 滚动      | p95 0.833µs / 百万行                         |
| WASM         | 372,852 / 393,216 gzip                       |
| ABI 版本     | 22（跨语言往返 + golden + fuzz 覆盖）        |

门禁全绿**不等于**可以发布：`gitTag` / `githubRelease` / `npmPublish` 在 M9-E 升级
发布流程前保持关闭，这是有意的。
